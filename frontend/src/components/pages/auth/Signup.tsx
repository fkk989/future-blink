...
import { handleSignup } from "../../../utils/helpers/auth";
...
export default function SignupPage() {
  ...
  const [errorState, setErrorState] = useState<SignUpData>({
    name: "",
    email: "",
    password: "",
  });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="w-[600px]  p-[40px] bg-white rounded-lg shadow">
        <h2 className="text-2xl font-bold mb-6 text-center">
          {showOtpFrom ? "Enter you OTP" : "signup"}
        </h2>
        {showOtpFrom ? (
          <OtpForm
          ...
          >
            <div className="flex flex-col gap-[10px]">
              ...
              <div
                className={`${
                  errorState.name ? "text-[tomato]" : "text-black"
                } h-[20px]`}
              >
                {errorState.name}
              </div>
        ...
        >
          {!showOtpFrom && (
            <div className="flex gap-[5px]">
              Already a user ?{" "}
              <p className="text-blue-500 hover:text-blue-400">Login</p>{" "}
            </div>
          )}
        </div>
      </div>
